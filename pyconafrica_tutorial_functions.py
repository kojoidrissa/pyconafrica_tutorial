...
def get_workbook_data(workbook):
    """
    Take a string (the name of an xls/xlsx workbook) and return the corresponding OpenPyXL workbook object
    ...
    # How could we deal with a changing sheet name?
    # demo_worksheet = wb.get_sheet_by_name("clean_data")
    demo_worksheet = wb["clean_data"]
    return wb
...
# output_book.save (filename = "OpenPyXL_demo_done.xlsx")
# print("File created: OpenPyXL_demo_done.xlsx ")
...
def main(workbook_file):
    assert isinstance(workbook_file, str)
    timesheet_data = get_workbook_data(workbook_file)
    print(type(timesheet_data))
    assert isinstance(timesheet_data, Workbook)
